...
/// The general pasteboard's change count, and whether its contents are marked
/// concealed, transient or auto-generated (<http://nspasteboard.org>) — what
/// 1Password, Bitwarden and Apple's Passwords put beside a copied password.
///
/// Reads neither the text nor anything else the system would ask permission
/// for: the count and the list of types are metadata.
#[must_use]
pub fn clipboard_state() -> (isize, bool) {
    ...
    let private = board.types().is_some_and(|types| {
        types
            .iter()
            .any(|kind| PRIVATE.contains(&kind.to_string().as_str()))
    });
    (board.changeCount(), private)
}

/// Ask for image files with the system's open panel; the paths chosen, or none.
///
/// Not the webview's `<input type="file">`: wry answers that with a bare
/// `runModal`, and this app is an accessory whose panel never activates it, so
/// the open panel came up in an inactive app — the file list ignored clicks
/// until changing folder woke it. Activating first makes the panel key.
///
/// Must run on the main thread; the caller marshals.
#[must_use]
pub fn pick_images() -> Vec<std::path::PathBuf> {
    ...
    // `activate` is macOS 14 and later; this still ships to 12.
    #[allow(deprecated)]
    ...
    // The four formats `images::sniff` stores. Deprecated for content types,
    // which would need another crate; `sniff` decides regardless.
    let types: Vec<_> = ["png", "jpg", "jpeg", "gif", "webp"]
        .into_iter()
