...
**Code blocks are fences, and the highlighter is ours.** ```` ```python ```` opens a block and ```` ``` ```` closes it, so a note with code in it is still Markdown on export and still plain text in the database. `lib/markdown.ts` grows a block pass (`parseBlocks`) that collapses a fenced run into one `code` block while every line keeps its index in the content, so ticking a checkbox below one still finds its own row. `lib/code.ts` is a lexer, not a parser: a table of languages (comment markers, quote characters, keyword lists, and flags for `property`, `markup`, `ignoreCase` and triple quotes) driving one loop. It knows nothing about scope or grammar and never fails — anything unrecognised stays plain, which is also how it degrades on a language it does not have. A library would have been a hundred kilobytes and a dependency outside brief section 4 to be right about constructs a ten-line snippet in a 320 px panel never shows. Every transform in `markdown.ts` refuses when the selection is inside a fence (`isInsideCode`): the point of a code block is that its text is exact, and a stray `**` from a mis-hit Cmd+B is part of the code from then on. The editor is a plain textarea, so it shows the fences and does not highlight; the reader and a locked card draw the block.
