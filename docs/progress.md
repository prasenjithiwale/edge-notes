...
Two things the owner reported, an hour apart.

**A status set in the sheet did not move the row.** Cancelling a task from its
details sheet left it under In progress until the tab was left and come back to.
That was `heldHere` doing exactly what it was written to do — and the rule was
wrong by one case. Holding a row is for the *box*: a press should not throw the
row into Done from under the finger that made it. A status chosen in the sheet is
an answer to "what is happening with this", and a list that ignores the answer
while the sheet that gave it is open inside the row looks broken, which is how it
was reported. `tickedHere` now only ever holds a *ticked* row, and the
scroll-into-view effect is keyed on the open task's status as well as its id, so
the sheet follows its row into the section it has moved to.

**A deleted note could not be found again.** Everything is soft-deleted and kept
for thirty days (brief 9.1) — but the undo toast was the only thing that ever
said so, so a delete noticed a minute late looked final. The rows were always
there; nothing could see them.

- `db/archive.rs` is one list rather than one per kind: the question is "where
  did the thing I just deleted go", and that is a question about *when*. Each row
  carries the note's whole text or the task's title (the frontend already knows
  how to make a heading and a preview out of a note), the note's colour id, and
  `purge_at` — sent rather than recomputed, so the screen and the startup purge
  cannot disagree about when something is gone for good.
- Restoring goes through `notes_restore` and `tasks_restore`, the commands undo
  already used: the archive is a second way to reach the same door, not a second
  door. It then re-reads that store, which is what puts a restored note back
  where its own `updated_at` says rather than at the top.
- `refreshArchive()` is the whole of the coupling: the delete and undo paths call
  it, and know nothing else about the archive.
- The toolbar button is **disabled while the archive is empty** and carries the
  count in its label ("Archive, 2 deleted"), so what it would show is known
  before it is pressed, and by a screen reader rather than only by eye. It shares
  the Esc cascade and the mutual exclusion with Settings: both are the panel
  showing you something about itself.

`IconButton` grew a `disabled` prop for this, dimmed rather than hidden — a
control that vanishes when it has nothing to do teaches nobody that it exists —
and its hover rule is now `:not(:disabled)`.

Not verified on screen: the machine was locked for both of these, so they were
checked by tests (74 in `Panel.test.tsx`, six in `store/archive.test.ts`, three
in `db/archive.rs`) rather than by clicking.

### Checklist

- [ ] Delete a note, let the toast go, open the archive: it is there
- [ ] Restore it: it returns to its old place in the list, not to the top
- [ ] Delete a task: it appears in the same list, marked as a task
- [ ] With nothing deleted, the archive button is dimmed and does nothing
- [ ] The button's tooltip counts what is in there
- [ ] Esc closes the archive and leaves the panel open
- [ ] Cancel a task from its sheet: the row leaves In progress at once
- [ ] Set one to In progress from the sheet: the row moves to the top and the
      sheet goes with it
- [ ] Tick a task: it stays where it is until the tab is left
...
## 24 Sep 2026 — Aurora glass (0.10.0)

The owner chose a mix of the Glass and Aura directions (mock-ups: https://claude.ai/artifact/A88smPukGXMEyaP42XivQe) and asked for it across the whole app, with calligraphy.

- **Glass:** `backdrop_set` + `platform::macos::set_backdrop` put one `NSVisualEffectView` (Popover material, Active state, appearance from the app's theme) behind the panel's rectangle once it is open; `useBackdrop` in `dock/glass.tsx` sends the rect and clears it on close. The panel draws `--glass-panel-solid` while sliding and `--glass-panel` once `data-backdrop` is set. Windows and Linux: solid tint.
- **Aura:** three blurred lights behind the masthead, one layer per mood (idle, focus, short, long), cross-faded; idle uses `--accent-bg`. Grain overlay on the panel. Reduced motion stops the drift and the orb's breathing.
- **Calligraphy:** Pinyon Script (bundled woff2, latin and latin-ext, ~55 KB, OFL) as `--font-script`: the masthead (view name plus today's date), the Focus phase word, and the empty states of Notes, Tasks, Clips and Archive.
- **Components:** every `--surface`/`--surface-sunken` background in the module sheets now uses `--glass-raised`/`--glass-well`; the slash and share menus use `--glass-popover` with an in-page blur. Tabs are a glass capsule, icon buttons are round and New is a raised glass button, note cards are tinted glass (`--card-tint`) with a lit edge and a glow of their own colour, the collapsed tab is a glass pill that glows while a session runs.
- **Focus:** the ring, its glow, the orb, the Start button and the phase word all take `--ring` from the phase (`--ring-focus`/`-short`/`-long`, each ≥ 4.5:1 against `--on-ink` because the button is filled with it).
- **Removed:** the Panel translucency slider (`panel.translucency` is still stored in Rust but no longer read), the flat accent band on the header and toolbar, and the Settings/Archive in-view headings (the masthead names them).
- **Not done:** the collapsed tab filling up as a session runs (it glows instead); a blur behind the tab pill (it is too small for its own effect view to be worth it).
- **Notes, option B "lit by its colour"** (mock-ups: https://claude.ai/artifact/E3hcKe12oeBRaT5eXB2HfA): cards, the editor, the large editor and the reader are the panel's glass (`--note-surface`) in the panel's ink, lit by the note's colour: a radial glow from the top-left (`--note-glow-mix`, 60% light / 42% dark) and a 2 px seam along the top, both from `--note-<c>-light`. All of it comes from `noteColorStyle`. Checkbox ticks are `--on-ink`; tool hovers are `--glass-well`; the archive's dot uses `--note-color`. "none" is unlit glass.
- **Notes, colour top to bottom** (owner, same day: only the top was coloured): a second glow from the bottom-right (`--note-glow-low`, 45% / 32%) and a flat wash of the colour over the whole note (`--note-wash`, 14% / 10%), on the card, editor and reader.
- **Task sheet and Clips in Aurora glass** (owner, same day, after 0.10.0): the task sheet is the note glass lit by its priority (`data-priority` → `--priority-*`, or the panel's light with none), with glass-capsule segments, pill chips filled in the ink when chosen, glass wells for the fields, a warm glass "Focus on this" and a filled Done. Clips rows are note-glass tiles; the current one is lit by `--accent-bg` (or the aura's lavender) like a note by its colour, instead of a solid accent band.

## 25 Sep 2026 — Download page in Aurora glass, with clips

- **New screenshots and three clips**, taken from the real app on invented data: `tools/site_demo.sh <scratch home>` writes a plaintext `notes.db` (six notes, six tasks, today's focus sessions, capture protection off in that database only), and a debug build run with `HOME=<scratch home> LEDGE_DB_KEY=<64 hex>` encrypts it with that key and never touches the keychain or the real notes. The panel was driven with `cliclick` over a designed backdrop page in Safari; stills with `screencapture -R`, clips with `screencapture -v -R`, converted to H.264 `.mp4` with the built-in `avconvert -p Preset1280x720`, posters pulled out with a few lines of AVFoundation. Keystrokes reach the panel only after a click *inside the note's text*; the + button alone is not enough.
- **`tools/render_site.sh`** now writes the page in the app's look: the aura drifting behind the page, glass header, cards, download tiles and clip frames, Pinyon Script for the wordmark, the hero, the section kickers and the changelog title (copied into `fonts/` with its licence). A "See it move" section plays `site/videos/*.mp4` only while on screen and never under reduced motion. Copy updated for 0.10: four tabs with Clips, encryption, the glass, presets; "the app asks the network for nothing" was untrue since the updater and now says what it does ask.
- **Found on the way:** the colour filter dots and the tab's dots used the palette's deep dark-mode backgrounds and vanished on dark glass; both now use `dotColor` (`lib/notes.ts`), the note's light. Settings' Panel colour description still said it tinted the header and toolbar. The code block's language picker shows "Plain text" for an alias such as `bash` (the highlighter knows it; the picker only lists `shell`) — not fixed yet.

## 25 Sep 2026 — Open source, and ready for SignPath Foundation

The owner is making this repository public and applying to SignPath Foundation for free Windows code signing. What that needed from here:

- **MIT licence** (`LICENSE`, `license` in `package.json` and `Cargo.toml`, `repository` in both), matching Tauri's own licensing; the bundle's copyright names the author. Pinyon Script stays under the OFL beside its files. Every dependency is open source (SignPath's rule is "no proprietary component").
- **Links follow the source.** Bug reports go to `edge-notes/issues`, not `edge-notes-apt/issues`; Settings › About gained a Source code row (four links now, and the test that used to forbid linking into this repository now requires it). The README, `CHANGELOG.md`, the site's generated README, `render_site.sh`, `publish_cask.sh` and `updates.rs` no longer call the repository private. Historical entries in this log are left as they were.
- **The download page has the sections SignPath requires**: `#signing` (their attribution line, what is signed and from where, manual approval, and the roles: Prasenjit Hiwale as committer, reviewer and approver) and `#privacy`. SignPath's stock privacy sentence ("will not transfer any information…") is not true of Ledge, which fetches `updates/latest.json` a minute after launch and daily, so the page says exactly that instead. The signing section says the application is pending and the Windows installers are unsigned until it is approved; **edit that sentence once it is.**
- **The Windows release job can sign** through `signpath/github-action-submit-signing-request@v3`: unsigned installers go up as a workflow artifact, come back signed, and the updater `.sig` files are made again from the signed bytes (`tauri signer sign`), because the ones Tauri wrote describe files that are no longer published. The three steps run only when the repository variable `SIGNPATH_ORGANIZATION_ID` exists; set it with `SIGNPATH_PROJECT_SLUG`, `SIGNPATH_SIGNING_POLICY_SLUG` and the secret `SIGNPATH_API_TOKEN` after approval. The job gained `actions: read` for SignPath to fetch the artifact. Every request waits for manual approval in SignPath, so a release now needs someone at the SignPath dashboard while the Windows job runs. **Not yet run for real.**

## 25 Sep 2026 — New app icon

The owner generated the artwork with ChatGPT from a brief: a frosted-glass pane docked to the right, its tab on the left edge, three glowing dots (coral, lavender, amber) over an aurora gradient, full-bleed with no shape of its own. It is kept as `tools/icon-source.png`.
